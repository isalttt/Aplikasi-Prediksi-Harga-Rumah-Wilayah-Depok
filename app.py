...
st.markdown("""
    ...
    /* FIX: Menghilangkan Navbar Atas & Tombol Deploy bawaan Streamlit */
    [data-testid="stHeader"] {
        display: none !important;
    }
    
    /* FIX OPTIMAL: Menghilangkan Tombol Panah Collapse (<<) di Pojok Kiri Atas Sidebar */
    [data-testid="sidebar-collapse-button"] {
        display: none !important;
    }
    
    /* Menyesuaikan jarak atas halaman agar tetap presisi dan rapi */
    .block-container {
...
for f in FITUR:
    # STRIP FIX: Membersihkan spasi atau enter (\n / \r) bawaan file pkl agar tidak memicu KeyError
    f_clean = f.strip()
    
    if 'Lokasi_encoded' in f_clean or f_clean == 'Lokasi_encoded':
    ...
    elif 'Luas Tanah' in f_clean or 'Luas Bangunan' in f_clean:
    ...
    elif any(keyword in f_clean for keyword in ['Kamar Tidur', 'Kamar Mandi', 'Garasi']):
        emoji = '🛏️' if 'Kamar Tidur' in f_clean else ('🚿' if 'Kamar Mandi' in f_clean else '🚗')
        input_user[f] = st.sidebar.number_input(
            label=f'{emoji} Total {f_clean}',
            min_value=0,
            value=2,
            step=1,
            format='%d'
        ...
        # Fallback handle untuk mengantisipasi sisa fitur tak terduga agar model tidak crash
        input_user[f] = 0
    ...
    if tombol_prediksi:
        try:
            ...
            if pred < 0:
                ...
                # Mengambil representasi nama kolom asli dari FITUR untuk dipetakan ke UI secara dinamis
                feat_lt = [k for k in FITUR if 'Luas Tanah' in k.strip()][0]
                feat_kt = [k for k in FITUR if 'Kamar Tidur' in k.strip()][0]
                feat_lb = [k for k in FITUR if 'Luas Bangunan' in k.strip()][0]
                feat_km = [k for k in FITUR if 'Kamar Mandi' in k.strip()][0]
                feat_gr = [k for k in FITUR if 'Garasi' in k.strip()][0]

                with col1:
                    st.markdown(f"<div class='grid-badge'><b>📍 Wilayah Kecamatan:</b><br>{lokasi_terpilih}</div>", unsafe_allow_html=True)
                    st.markdown(f"<div class='grid-badge'><b>📐 Luas Bidang Tanah:</b><br>{input_user[feat_lt]} m²</div>", unsafe_allow_html=True)
                with col2:
                    st.markdown(f"<div class='grid-badge'><b>🛏️ Kamar Tidur Utama:</b><br>{input_user[feat_kt]} Ruang</div>", unsafe_allow_html=True)
                    st.markdown(f"<div class='grid-badge'><b>📐 Luas Fisik Bangunan:</b><br>{input_user[feat_lb]} m²</div>", unsafe_allow_html=True)
                with col3:
                    st.markdown(f"<div class='grid-badge'><b>🚿 Kamar Mandi Bersih:</b><br>{input_user[feat_km]} Ruang</div>", unsafe_allow_html=True)
                    st.markdown(f"<div class='grid-badge'><b>🚗 Kapasitas Area Garasi:</b><br>{input_user[feat_gr]} Kendaraan</div>", unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)

        except Exception as e:
            st.error(f'Terjadi gangguan teknis sistem: {e}')
    else:
        # FIX: Tanda emoji penunjuk (👈) di bawah ini sudah dibersihkan total
        st.markdown(f"""
...
with tab2:
    ...
    with c_table:
        ...
        df_koef = pd.DataFrame({
            'Karakteristik Properti': [f.strip() for f in FITUR],
